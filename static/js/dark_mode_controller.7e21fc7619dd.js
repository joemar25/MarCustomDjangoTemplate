...
export function updateThemeToggleIcons(darkModeEnabled) {
  ...
  // Concisely toggle icon visibility using Tailwind classes
  darkIcon.classList.toggle("hidden", !darkModeEnabled);
  lightIcon.classList.toggle("hidden", darkModeEnabled);

  // Ensure consistency after potential htmx updates (if applicable)
  if (typeof htmx !== "undefined") {
    htmx.on("htmx:afterSwap", () => {
      updateThemeToggleIcons(darkModeEnabled); // Re-apply updates
    });
  }
}
